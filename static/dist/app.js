...
var GED_CONFIG = {
  ...
  rust_bundler: true,
  // env20 has Goo's native bundler installed
  repos: [
    {
      id: "community",
      path: "/home/odoo/work/community",
...
var CodePlugin = class extends Plugin {
  ...
  pullRequestUrl(github, number) {
    return this.config.repoByGithub(github)?.pullRequestUrl(number) ?? repoUrls.pullRequest(github, number);
  }
...
var ALIASES = {
  ...
  addons: "workspaces",
  prs: "branches",
  // the PRs screen merged into "Branches & PRs"
  reviews: "branches"
  // the PR review feature retired; same merged screen
};
...
var WorkspacePlugin = class extends Plugin {
  ...
  async createWorktree({
    ...
    const repos = checkouts.map(
      ({ repo, branch }) => forkRepos.has(repo) ? {
        repo,
        newBranch: branch,
...
function savedCollapsed(stateKey) {
  if (!stateKey) return /* @__PURE__ */ new Set();
  try {
...
var RecordList = class extends Component {
  ...
  get groups() {
    ...
    const groups = [...map.entries()].map(([key, rows2]) => ({
      key,
      label: this.props.groupLabel ? this.props.groupLabel(key, rows2) : key,
      rows: rows2,
  ...
  label(f) {
    return f.label === void 0 ? f.name : f.label;
  }
...
var RemoteBranchDialog = class extends Component {
  ...
  async _searchRemote(query) {
    const ids = this.repoIds;
    const repos = this.config.config.repos.filter((r) => r.github && (!ids || ids.has(r.id))).map((r) => ({
      id: r.id,
      github: r.github,
      path: r.path,
    ...
    try {
      ...
      for (const { repo, branch, remote } of data.results) {
        if (!byBranch.has(branch)) byBranch.set(branch, /* @__PURE__ */ new Map());
...
var BranchesScreen = class extends Component {
  ...
  // merge status; "" = all, "unmerged" = still in flight
  selected = signal(/* @__PURE__ */ new Set());
  // branch names ticked for batch actions
  sortDir = signal("desc");
  // groups by combined last-update time; "asc" | "desc"
  sortKey = signal("update");
  // the one sortable column (kept as state for the header API)
  ...
  get count() {
    ...
    return `${branches} branch${branches === 1 ? "" : "es"} \xB7 ${prs.size} PR${prs.size === 1 ? "" : "s"}`;
  }
  ...
  const res = await dialogs.open({
    ...
    fields: [
      ...
      },
      {
        ...
        hint: (values) => {
          const checkouts2 = repoBranchList.parse((values.config || "").trim());
          if (!checkouts2.length) return null;
          return checkouts2.map(
            (c) => hasLocalBranch(c.repo, c.branch) ? `${c.repo}: attach existing "${c.branch}"` : `${c.repo}: fork new from ${baseBranchOf(c.branch)}`
          ).join(" \xB7 ");
        }
      },
      {
        ...
        run: async (values) => {
          ...
          for (const repoId of toFetch) {
            const r = await fetchRemoteBranch(dialogs, {
              ...
              branch: res2.branch,
              pull_remote: res2.remoteByRepo[repoId] || pullRemoteByRepo[repoId]
            });
          ...
          const repos = [.../* @__PURE__ */ new Set([...values.repos, ...res2.repos])];
          const currentByRepo = Object.fromEntries(
            repoBranchList.parse(values.config || "").map((c) => [c.repo, c.branch])
          );
          const config2 = repoBranchList.format(
            repos.map((repo) => ({
              repo,
              branch: res2.repos.includes(repo) ? res2.branch : currentByRepo[repo] ?? res2.branch
            }))
          );
          return { name: res2.branch, repos, config: config2 };
        }
      },
      {
        key: "db",
...
var TodoScreen = class extends Component {
  ...
  toggleStar(id) {
    this._updateTodos(
      (todos) => todos.map((todo) => todo.id === id ? { ...todo, starred: !todo.starred } : todo)
    );
  }
  openTodo(id) {
...
var CommitHistory = class extends Component {
  ...
  get historyDiffLines() {
    return this.historyDiff().split("\n").filter((text) => !text.startsWith("diff --git ") && !text.startsWith("index ")).map((text, id) => {
      let cls = "";
  ...
  get historyPlanSummary() {
    ...
    else if (this.historyPlanIds().join("\0") !== this.props.commits.filter((commit) => commit.ahead).map((commit) => commit.sha).join("\0")) {
...
var CodePane = class extends Component {
  ...
  // the heading's "N prs" link-copier
  menuId = signal("");
  // key of the checkout whose action menu is open ("" = none)
  prsCopied = signal(false);
  // transient "copied" label on the PR-link copier
  rPlusPosts = signal({});
  // "github#number" -> "posting" | "posted"
  ...
  get checkoutRows() {
    ...
    return (view.checkouts || []).map((c) => {
      ...
      return {
        ...
        synced: !!(b && b.remote && b.synced),
        // …and the local tip is what's on the remote
        subject: b?.subject || "",
        sha: b?.sha || "",
...
var WorkspacesScreen = class extends Component {
  ...
  get listGroups() {
    ...
    if (!this.categoriesEnabled) {
      ...
      const cats = (this.config.config.workspace_categories || []).map((c) => c.id).filter((c) => c !== ARCHIVED_CATEGORY);
