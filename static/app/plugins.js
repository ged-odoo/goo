...
const { Plugin, proxy, plugin, effect } = owl;
...
export class ConfigPlugin extends Plugin {
  ...
  setup() {
    this.state = proxy({ config: this._merged(), dataFile: this.getDataFile() });
    this._timer = null;
  }
...
export class RouterPlugin extends Plugin {
  ...
  static sequence = 1;
  setup() {
    this.state = proxy({ section: this._fromHash() });
...
export class ServerPlugin extends Plugin {
  ...
  setup() {
    ...
    this.state = proxy({ status: { state: "stopped" }, now: Date.now() });
...
export class DatabasePlugin extends Plugin {
  ...
  setup() {
    ...
    this.state = proxy({ databases: [], at: 0, loading: false, error: "", dropping: "" });
...
export class TestsPlugin extends Plugin {
  ...
  setup() {
    this.server = plugin(ServerPlugin);
    this.state = proxy({ status: "", runActive: false, sawRun: false });
