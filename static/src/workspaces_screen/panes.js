// The Workspaces screen's Tests / Addons / Assets tab panes — slim, per-workspace
// versions of the standalone screens, addressing the plugins' per-slot state
// ("main" for a main-located workspace, the workspace id for a worktree). The
// standalone screens keep the main slot and are retired in Phase 5.
...
export class AssetsPane extends Component {
  ...
  setup() {
    // point the (db-scoped) assets plugin at this workspace's db. selectedDb is
    // read/written UNTRACKED: a t-key workspace switch briefly leaves the outgoing
    // pane's effect alive (owl destroys it deferred), and a tracked read would let
    // the two instances re-trigger each other over the shared signal forever — a
    // microtask loop that starves the event loop and freezes the page.
    useEffect(() => {
      const db = this.props.ws.db;
